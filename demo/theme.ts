...
export class XDemo extends LitElement {
  ...
  render() {
    return html`
      <exmg-paper-sidemenu .menu=${menu} selected=${this.selected} @selected-change=${(e:CustomEvent) => this.selected = e.detail} debug>
        <exmg-paper-sidemenu-header slot="header" selected=${this.selected} @selected-change=${(e:CustomEvent) => this.selected = e.detail} debug></exmg-paper-sidemenu-header>
      </exmg-paper-sidemenu>
    `;
  }
}